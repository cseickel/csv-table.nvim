...
M.actions = {
  select_cell = action("Select this cell", function(buf)
  ...
  select_row = action("Select this whole row", selector(function(buf, painted)
    local cell = cursor.cell_ref(buf, 0)
    if not cell then
      return nil, nil
    end
  ...
  select_column = action("Select this whole column", selector(function(buf, painted)
    local cell = cursor.cell_ref(buf, 0)
    if not cell then
      return nil, nil
    end
  ...
  select_page = action("Select every cell on this page", selector(function(buf, painted)
    return
      { row = painted.rowids[painted.first_row], column = 1 },
      { row = painted.rowids[painted.last_row], column = last_column(buf) }
  end)),

  clear_selection = action("Select nothing", function(buf)
    range.clear(buf.state)
    buffer.repaint(buf)
  end),

  extend_left = action("Take the selection one column left", extender(0, -1)),
  extend_right = action("Take the selection one column right", extender(0, 1)),
  extend_up = action("Take the selection one row up", extender(-1, 0)),
  extend_down = action("Take the selection one row down", extender(1, 0)),

  extend_to_first_column = action("Take the selection to the first column", extender(0, -EDGE)),
  extend_to_last_column = action("Take the selection to the last column", extender(0, EDGE)),
  extend_to_first_row = action("Take the selection to the top of the page", extender(-EDGE, 0)),
  extend_to_last_row = action("Take the selection to the bottom of the page", extender(EDGE, 0)),

  copy = action("Copy the selected cells, or this one", function(buf)
    inspect.copy(buf)
  end),
}
