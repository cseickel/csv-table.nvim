...
- `info` gives the source, the row and column counts, the filters, the sort, and a
  line per column
...
--- Every action, sorted by its description. Sorting by key would put `sa` next to
--- `sd` and `ss`, which do three different things. An action with no key is in the
--- list too, because the help panel runs whatever the user picks.
---@return csv.Key[]
function M.bindings()
...
local function query_lines(buf)
  ...
  for _, filter in ipairs(buf.query.filters) do
    ...
    elseif filter.type == "in" then
      table.insert(
        lines,
        string.format("  %s is one of %s", filter.column.label, table.concat(filter.values, ", "))
      )
    else
      table.insert(
        lines,
        string.format("  %s %s %s", filter.column.label, filter.operator, filter.value)
      )
    end
  end
  ...
  for index, column in ipairs(buf.query.file.columns) do
    ...
    table.insert(
      ...
      string.format(
        ...
        column.label,
        column_format.kind,
        column_format.precision,
        column_format.align or "auto",
        summary.min or "",
        summary.max or "",
...
function M.stats(buf, column)
  buf.query.reader:stats(buf.query, column, function(rows)
    ...
    popup.open(lines, { title = column.label })
  end)
end
